...
const ELITE_CLUBS_KEYWORDS: readonly string[] = [
  ...
  'juventus', 'ac milan', 'inter milan', 'internazionale', 'fc internazionale', 'napoli',
  'paris saint-germain', 'paris sg', 'psg', 'olympique de marseille',
  'ajax', 'porto', 'benfica', 'sporting cp', 'sporting lisboa',
  'celtic', 'rangers',
...
export function computeFanMoodDelta(
  ...
  if (isLoss) {
    let base: number;
    if (isClassico) base = -16;
    else if (opponentScore >= 4) base = isEliteOpponent ? -10 : -14;
    else base = isEliteOpponent ? -6 : -9;
    if (prestige === 'large') return Math.round(base * 1.1);
    return base;
  }

  if (isDraw) return unbeatenStreak >= 5 ? +1 : 0;
