...
function computeNewMood(stats: ExtendedPlayerSeasonStats): Mood {
  ...
  if ((stats.matchesAsSubstitute ?? 0) > (stats.matchesAsStarter ?? 0) && totalApps >= 5 && recentAvg < 6.5) {
    delta -= 1;
  }
...
function computeNewFanMoral(stats: ExtendedPlayerSeasonStats): FanMoral {
...
export function runPerformanceEngine(seasonId: string): Promise<void> {
  ...
  return mutated ? syncAllPlayerStats(seasonId) : Promise.resolve();
}
