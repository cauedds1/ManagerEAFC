// PORTED FROM artifacts/fc-career-manager/src/lib/boardMoodStorage.ts — adapted for React Native (AsyncStorage-backed localCache, no DOM).
...
export function computeBoardMoodDelta(params: BoardMoodDeltaParams): number {
  ...
  if (isLoss && projeto) {
    ...
    if (isTitle) delta = Math.floor(delta * 1.2);
