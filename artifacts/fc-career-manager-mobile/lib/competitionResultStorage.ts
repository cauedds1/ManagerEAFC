...
export function deleteCompetitionResult(careerId: string, resultId: string): void {
  _save(careerId, getCompetitionResults(careerId).filter((r) => r.id !== resultId));
}

export function hydrateCompetitionResultsCache(careerId: string, data: Record<string, unknown>): void {
  if (Array.isArray(data['comp_results'])) {
    sessionSet(storageKey(careerId), data['comp_results']);
  }
}

export function generateResultId(): string   { return `cr-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`; }
export function generateRoundId(): string    { return `rnd-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 5)}`; }
export function generateMatchId(): string    { return `bm-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 5)}`; }
export function generateStandingId(): string { return `st-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 5)}`; }
