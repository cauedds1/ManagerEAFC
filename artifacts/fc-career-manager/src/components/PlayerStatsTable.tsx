...
function Th({ label, col, sortCol, asc, onSort, title }: ThProps) {
  const active = sortCol === col;
  return (
    <th
      className="px-2 py-2 text-center cursor-pointer select-none whitespace-nowrap"
...
export function PlayerStatsTable({ careerId, allPlayers }: Props) {
  ...
  const rows: Row[] = useMemo(() => {
    return allPlayers
      ...
      .map((p) => ({ player: p, stats: rawStats[p.id] }));
  }, [allPlayers, rawStats]);
  ...
  const sorted = useMemo(() => {
    ...
    return [...rows].sort((a, b) => {
      ...
      switch (sortCol) {
        ...
        case "number":  diff = (a.player.number ?? 99) - (b.player.number ?? 99); break;
        ...
        case "overall": {
          const ovA = a.player.number ?? 0;
          const ovB = b.player.number ?? 0;
          diff = ovA - ovB;
          break;
        }
      }
        ...
        <thead>
          <tr style={{ background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            <th className="px-3 py-2 text-left w-8">
              <span className="text-xs text-white/30 font-semibold">#</span>
            </th>
            <Th label="Jogador" col="name" {...thProps} />
          ...
          {sorted.map(({ player, stats }, i) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-2 py-2.5 text-center text-xs tabular-nums" style={{ color: stats.yellowCards > 0 ? "#fbbf24" : "rgba(255,255,255,0.3)" }}>
                  {stats.yellowCards > 0 ? (
                ...
                <td className="px-2 py-2.5 text-center text-xs tabular-nums" style={{ color: stats.redCards > 0 ? "#f87171" : "rgba(255,255,255,0.3)" }}>
                  {stats.redCards > 0 ? (
                    <span className="inline-flex items-center gap-1">
