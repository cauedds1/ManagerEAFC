...
const EXTRA_ICONS: Record<"pro" | "ultra", React.ReactNode> = {
  pro: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export function WelcomeScreen({ userName, plan, onContinue }: WelcomeScreenProps) {
  ...
  const planBadge = plan === "free" ? t.planFree : plan === "pro" ? "FC Career Pro" : "FC Career Ultra";

  const feat4Desc = plan === "free" ? t.feat4DescFree : plan === "pro" ? t.feat4DescPro : t.feat4DescUltra;

  const baseFeatures = [
    { icon: FEAT_ICONS[0], title: t.feat1Title, desc: t.feat1Desc },
    { icon: FEAT_ICONS[1], title: t.feat2Title, desc: t.feat2Desc },
    { icon: FEAT_ICONS[2], title: t.feat3Title, desc: t.feat3Desc },
    { icon: FEAT_ICONS[3], title: t.feat4Title, desc: feat4Desc },
  ];

  const extraText = plan === "pro" ? t.extraPro : plan === "ultra" ? t.extraUltra : null;
  const extraIcon = plan !== "free" ? EXTRA_ICONS[plan] : null;
      ...
      >
          ...
          {baseFeatures.map((feat, i) => (
            ...
            >
              ...
              <div>
                ...
                <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 11, lineHeight: 1.5 }}>{feat.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {extraText && extraIcon && (
          ...
          >
            ...
            <p style={{ color: cfg.color, fontSize: 13, fontWeight: 600, lineHeight: 1.4 }}>{extraText}</p>
          </div>
        )}
