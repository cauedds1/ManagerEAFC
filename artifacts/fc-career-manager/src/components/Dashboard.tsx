...
function PlayerPhoto({ src, name }: { src: string; name: string }) {
  ...
  return (
    <div className="w-10 h-10 rounded-full flex-shrink-0 overflow-hidden flex items-center justify-center" style={{ background: "rgba(255,255,255,0.06)" }}>
      {!error ? (
        <img
          src={src}
          alt={name}
  ...
  return (
    <section className="max-w-6xl mx-auto w-full px-4 sm:px-6 pb-8">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 rounded-2xl gap-3" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
          ...
          <button onClick={handleRefresh} className="px-4 py-2 rounded-xl text-xs font-semibold text-white hover:opacity-80" style={{ background: "var(--club-primary)" }}>Tentar novamente</button>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-12 rounded-2xl gap-3" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
          ...
          {!hasApiKey && (
            <button onClick={onOpenSettings} className="px-4 py-2 rounded-xl text-xs font-semibold text-white hover:opacity-80" style={{ background: "var(--club-primary)" }}>Configurar API key</button>
          )}
        </div>
      ) : (
        <>
          ...
          {!hasApiKey && squad.source === "fc26" && (
            <div className="mt-4 flex items-center gap-2 px-4 py-3 rounded-xl" style={{ background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.15)" }}>
              <svg className="w-4 h-4 flex-shrink-0" style={{ color: "#818cf8" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "var(--app-bg, #0a0a0a)" }}>
      ...
      <header className="relative w-full overflow-hidden" style={{ background: "var(--club-gradient)" }}>
        ...
        <div
          ...
          style={{
            backgroundImage: `radial-gradient(circle at 15% 50%, var(--club-primary) 0%, transparent 55%), radial-gradient(circle at 85% 50%, var(--club-secondary) 0%, transparent 55%)`,
          }}
        />
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
          ...
          <div className="flex items-center gap-2 mb-6">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-white/50 text-xs font-medium truncate max-w-32">
                {career.coach.name}
              </span>
            </div>
          </div>
          ...
          {statCards.map((card) => (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-[var(--club-primary)]" style={{ background: "var(--club-primary)18" }}>
                  {card.icon}
                </div>
              </div>
              <div>
      ...
      <section className="max-w-6xl mx-auto w-full px-4 sm:px-6 pb-6">
        <h2 className="text-white/40 text-xs font-semibold tracking-widest uppercase mb-4">Técnico</h2>
        <div
      ...
      <section className="max-w-6xl mx-auto w-full px-4 sm:px-6 pb-12">
        <div className="flex items-center gap-3 mb-5">
          <h2 className="text-white/40 text-xs font-semibold tracking-widest uppercase">Próximas funcionalidades</h2>
          <div className="flex-1 h-px" style={{ background: "rgba(255,255,255,0.06)" }} />
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: "var(--club-primary)20", color: "var(--club-primary)" }}>
            Em breve
          </span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {ROADMAP.map((item) => (
            <div
              ...
              style={{ background: `linear-gradient(135deg, var(--club-primary)${item.accentOpacity}, var(--club-secondary)0a)`, border: "1px solid rgba(255,255,255,0.06)" }}
            >
              <div className="flex-shrink-0">
                <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold" style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.7)" }}>
                  {item.fase}
                </span>
              </div>
              <div>
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2">
                <svg className="w-4 h-4 text-white/20" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
              </div>
            </div>
          ))}
        </div>
      ...
      <Settings
        ...
        onReloadClubs={() => { setSettingsOpen(false); onReloadClubs(); }}
      />
    </div>
  );
}
