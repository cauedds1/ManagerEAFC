...
function PlayerPhoto({ src, name }: { src: string; name: string }) {
  ...
  return (
    <div className="w-10 h-10 rounded-full flex-shrink-0 overflow-hidden flex items-center justify-center" style={{ background: "rgba(var(--club-primary-rgb),0.06)" }}>
      {!error ? (
        <img src={src} alt={name} className={`w-10 h-10 object-cover transition-opacity duration-200 ${loaded ? "opacity-100" : "opacity-0"}`} onLoad={() => setLoaded(true)} onError={() => setError(true)} />
      ) : (
...
function PlayerRow({ player }: { player: SquadPlayer }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 rounded-xl glass glass-hover transition-all duration-200">
  ...
  return (
    <div className="relative min-h-screen flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div className="flex items-center gap-5">
              <div className="w-20 h-20 rounded-2xl flex items-center justify-center flex-shrink-0"
                ...
                {logoUrl && !imgError ? (
                  ...
                  <span className="text-3xl font-black text-white/40">{career.clubName.substring(0, 2).toUpperCase()}</span>
                )}
              </div>

              <div>
                <p className="text-xs font-semibold tracking-widest uppercase mb-1" style={{ color: "var(--club-primary)" }}>
                  Gerenciador de Carreira
                </p>
                <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">{career.clubName}</h1>
                <p className="text-white/50 text-sm mt-1">{career.clubLeague}</p>
                {career.clubCountry && <p className="text-white/30 text-xs mt-0.5">{career.clubCountry}</p>}
            ...
            <div className="flex flex-col items-start sm:items-end gap-3">
              <div className="flex items-center gap-2">
                <span className="text-white/40 text-xs">Temporada:</span>
                {editingSeason ? (
                  <input autoFocus value={seasonDraft}
                    ...
                    className="px-3 py-1 rounded-lg text-white font-bold text-sm focus:outline-none glass"
                    style={{ minWidth: "80px", boxShadow: "0 0 12px rgba(var(--club-primary-rgb),0.2)" }}
                  />
                ) : (
                  <button onClick={() => setEditingSeason(true)}
                    className="flex items-center gap-1.5 px-3 py-1 rounded-lg font-bold text-sm text-white hover:bg-white/10 transition-colors duration-200 group glass">
                    {career.season}
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setSettingsOpen(true)}
                  className="flex items-center justify-center w-10 h-10 rounded-xl text-white/50 hover:text-white transition-all duration-200 glass glass-hover"
                  title="Configuracoes">
                ...
                <button onClick={onChangeClub}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm text-white transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] glass glass-hover">
      ...
      <section className="max-w-6xl mx-auto w-full px-4 sm:px-6 py-8">
        <h2 className="text-white/40 text-xs font-semibold tracking-widest uppercase mb-5">Visao Geral</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {statCards.map((card) => (
            <div key={card.label} className="relative flex flex-col gap-4 p-6 rounded-2xl overflow-hidden group glass glass-hover transition-all duration-200">
              <div className="flex items-center justify-between">
                <span className="text-white/40 text-sm font-medium">{card.label}</span>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "rgba(var(--club-primary-rgb),0.1)", color: "var(--club-primary)" }}>
                  {card.icon}
                </div>
              </div>
              <div>
      ...
      <section className="max-w-6xl mx-auto w-full px-4 sm:px-6 pb-6">
        <h2 className="text-white/40 text-xs font-semibold tracking-widest uppercase mb-4">Tecnico</h2>
        <div className="flex items-center gap-4 p-5 rounded-2xl glass glass-hover transition-all duration-200">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-xl">{career.coach.nationalityFlag}</span>
              <h3 className="text-white font-black text-lg truncate">{career.coach.name}</h3>
            </div>
            <p className="text-white/40 text-sm">{career.coach.nationality} · {career.coach.age} anos</p>
          </div>
          <div className="text-right flex-shrink-0">
            <p className="text-white/20 text-xs">Carreira desde</p>
            <p className="text-white/40 text-xs">{formatDate(career.createdAt)}</p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto w-full px-4 sm:px-6 pb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {ROADMAP.map((item) => (
            <div key={item.fase} className="relative flex gap-4 p-5 rounded-2xl overflow-hidden glass glass-hover transition-all duration-200">
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2">
                <svg className="w-4 h-4 text-white/15" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
              </div>
            </div>
          ))}
        </div>
      ...
      <Settings isOpen={settingsOpen} onClose={() => setSettingsOpen(false)} onReloadClubs={() => { setSettingsOpen(false); onReloadClubs(); }} />
    </div>
  );
}
